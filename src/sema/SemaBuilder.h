...
public :
		...
		std::any visitConstantDecl(ClearLanguageParser::ConstantDeclContext* context) override;
...
public:
    ...
    using value_variant = std::variant<std::monostate, int64_t, uint64_t, double, bool, std::string>;
        ...
        struct symbol_entry {
            ...
            [[nodiscard]] bool has_folded() const noexcept { return !std::holds_alternative<std::monostate>(value); }
        };
...
private:
    ...
    void push_scope(const scope_kind k) { symbol_scopes_.push_back(scope_frame{ k, {} }); }
    void pop_scope(){
        if (symbol_scopes_.empty()) throw std::runtime_error("symbol scope underflow");
        symbol_scopes_.pop_back();
    }

    bool insert_symbol(const std::string& name, symbol_entry& entry);

	[[nodiscard]] const symbol_entry* lookup_symbol(const std::string& name) const {
	...
	[[nodiscard]] const symbol_entry* lookup_function_symbol(const std::string& name) const;
	[[nodiscard]] type_ref resolve_type_symbol(const std::string& name) const;

    bool try_fold_expr(const std::shared_ptr<sema::expr>& e, value_variant& out) const;
    ...
    struct scope_guard {
        ...
        bool active{ true };
        scope_kind kind;
        explicit scope_guard(sema_builder& b, scope_kind k) : sb(b), kind(k) { sb.push_scope(k); }
        ~scope_guard() { if (active) sb.pop_scope(); }
    ...
    struct file_ctx_guard {
        ...
        file_ctx_guard(sema_builder& b, ClearLanguageParser::StartContext* ctx) : sb(b), prev_package(b.current_package_), prev_imports(b.imports_) {
	...
	[[nodiscard]] const symbol_entry* lookup_symbol_qualified(const std::string& name) const {
        auto q = name;
        auto fqn = qualify(q);
