...
#elif defined(__FLT16_MANT_DIG__) || defined(__STD_FLOAT16_T__) || \
    defined(__MSC_VER)
...
struct type {
  ...
  [[nodiscard]] bool is_unsigned() const {
    return kind == kind_enum::u8 || kind == kind_enum::u16 ||
           kind == kind_enum::u32 || kind == kind_enum::u64;
  }
...
struct value {
  type_ref type;
  std::variant<int64_t, uint64_t, std::monostate, function_value, cl_half,
               std::string, cl_f32>
      v;
  bool is_untyped_int = false;
  value() = default;
  value(const type_ref& type,
        std::variant<int64_t, uint64_t, std::monostate, function_value, cl_half,
                     std::string, cl_f32>
            value,
        bool cond);
...
inline value::value(const type_ref& type,
                    std::variant<int64_t, uint64_t, std::monostate,
                                 function_value, cl_half, std::string, cl_f32>
                        value,
                    const bool cond)
    : type(type), v(std::move(value)), is_untyped_int(cond) {}

inline bool is_numeric(const value& val) {
  return std::holds_alternative<int64_t>(val.v) ||
         std::holds_alternative<uint64_t>(val.v);
}

inline std::variant<int64_t, uint64_t> as_num2(const value& v) {
  if (std::holds_alternative<int64_t>(v.v))
    return std::variant<int64_t, uint64_t>(std::get<int64_t>(v.v));
  if (std::holds_alternative<uint64_t>(v.v))
    return std::variant<int64_t, uint64_t>(std::get<uint64_t>(v.v));
...
inline bool fits(const type_ref& t, const std::variant<int64_t, uint64_t>& v) {
  if (!t.is_builtin())
    throw std::runtime_error(
        "numeric property requested on non-numeric type: " + type_name(t));
...
inline value make_string(std::string s) {
  return value{type_ref::builtin_type(type{type::kind_enum::string}),
               std::move(s), false};
}
...
inline value concat_string(const value& lhs, const value& rhs) {
  if (!is_string(lhs.type) || !is_string(rhs.type))
    throw std::runtime_error(
        "type mismatch: string concat requires string + string");
