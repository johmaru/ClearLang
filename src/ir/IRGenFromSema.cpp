...
ir_gen_from_sema::ir_gen_from_sema(llvm::LLVMContext& ctx,
...
void ir_gen_from_sema::emit_entry_shim(const sema::module& m) const {
  ...
  auto* data_ptr =
      builder_->CreateGEP(i8_ty, out, llvm::ConstantInt::get(i32_ty, 1));
  ...
  if (ret_ty->isIntegerTy(8)) {
    ...
    builder_->CreateRet(
        llvm::ConstantInt::get(i32_ty, 2));  // 1(tag) + 1 (data)
    return;
  }
  if (ret_ty->isIntegerTy(16)) {
    ...
    builder_->CreateRet(
        llvm::ConstantInt::get(i32_ty, 3));  // 1(tag) + 2 (data)
    return;
  }
  if (ret_ty->isIntegerTy(32)) {
    ...
    builder_->CreateRet(
        llvm::ConstantInt::get(i32_ty, 5));  // 1(tag) + 4 (data)
    return;
  }
  if (ret_ty->isIntegerTy(64)) {
    llvm::Value* v = builder_->CreateCall(entry, {});
    store_tag(5);
    auto* p64 = builder_->CreateBitCast(
        data_ptr, llvm::Type::getInt64Ty(c)->getPointerTo());
    builder_->CreateStore(v, p64);
    builder_->CreateRet(
        llvm::ConstantInt::get(i32_ty, 9));  // 1(tag) + 8 (data)
    return;
  }
  if (ret_ty->isHalfTy()) {
  ...
  if (auto* vd = dynamic_cast<const sema::stmt_var_decl*>(&s)) {
      ...
      if (vd->decl_type.builtin.kind == type::kind_enum::string)
        init = llvm::ConstantPointerNull::get(
            llvm::Type::getInt8Ty(ctx_)->getPointerTo());
               ...
               vd->decl_type.builtin.kind == type::kind_enum::f32)
        init = llvm::ConstantFP::get(to_llvm_type(vd->decl_type),
                                     llvm::APFloat(0.0f));
    ...
    if (cond_v->getType()->isIntegerTy(1)) {
      ...
      cond_i1 = builder_->CreateICmpNE(
          cond_v, llvm::ConstantInt::get(cond_v->getType(), 0));
    } else if (cond_v->getType()->isHalfTy() ||
               cond_v->getType()->isFloatTy()) {
      cond_i1 = builder_->CreateFCmpONE(
          cond_v, llvm::ConstantFP::get(cond_v->getType(), 0.0));
    } else if (cond_v->getType()->isPointerTy()) {
      cond_i1 = builder_->CreateICmpNE(
          cond_v, llvm::ConstantPointerNull::get(
                      llvm::cast<llvm::PointerType>(cond_v->getType())));
    } else {
...
llvm::Value* ir_gen_from_sema::emit_expr(const sema::expr& e) {
  ...
  if (auto* vr = dynamic_cast<const sema::var_ref*>(&e)) {
    ...
    if (it == vars_.back().end())
      throw std::runtime_error("undefined var in IRGen: " + vr->name);
    return it->second;
  }
  ...
  if (auto* c = dynamic_cast<const sema::call*>(&e)) {
    if (c->callee == "__cl_f16_printfn" || c->callee == "__cl_f32_printfn") {
      ...
      if (arg0->getType()->isHalfTy()) {
        ...
        throw std::runtime_error(fn_name +
                                 ": doesn't support other then float type");
  ...
  if (auto* cast = dynamic_cast<const sema::cast*>(&e)) {
    ...
    if (is_int_kind(src.builtin.kind) &&
        dst.builtin.kind == type::kind_enum::f16) {
      if (!v->getType()->isIntegerTy() || !dst_ty->isHalfTy())
        throw std::runtime_error("emitCast(int<->f16): type mismatch");

      return src.builtin.is_unsigned() ? builder_->CreateUIToFP(v, dst_ty)
                                       : builder_->CreateSIToFP(v, dst_ty);
    }

    if (src.builtin.kind == type::kind_enum::f16 &&
        is_int_kind(dst.builtin.kind)) {
      if (!v->getType()->isHalfTy() || !dst_ty->isIntegerTy())
        throw std::runtime_error("emitCast(f16->int): type mismatch");
      return dst.builtin.is_unsigned() ? builder_->CreateFPToUI(v, dst_ty)
                                       : builder_->CreateFPToSI(v, dst_ty);
    }

    if (is_int_kind(src.builtin.kind) &&
        dst.builtin.kind == type::kind_enum::f32) {
    ...
    if (src.builtin.kind == type::kind_enum::f32 &&
        is_int_kind(dst.builtin.kind)) {
...
llvm::Value* ir_gen_from_sema::emit_bin_op(const sema::bin_op& b) {
  ...
  if (b.op == "and" || b.op == ("or")) {
    ...
    auto to_i1 = [&](llvm::Value* v) -> llvm::Value* {
      ...
      throw std::runtime_error(
          "emitBinOp(and|or): cannot coerce operand to i1");
    };
  ...
  if (lhs->getType()->isIntegerTy() && lhs->getType() == rhs->getType()) {
    const bool is_unsigned =
        b.lhs->type.builtin.is_unsigned() && b.rhs->type.builtin.is_unsigned();
    ...
    if (b.op == "/")
      return is_unsigned ? builder_->CreateUDiv(lhs, rhs)
                         : builder_->CreateSDiv(lhs, rhs);
    if (b.op == "%")
      return is_unsigned ? builder_->CreateURem(lhs, rhs)
    ...
    if (!f) {
      f = llvm::Function::Create(fnt_ty, llvm::Function::ExternalLinkage,
...
llvm::Constant* ir_gen_from_sema::to_llvm_const(const value& v) const {
  if (!v.type.is_builtin())
    throw std::runtime_error("toLlvmConst: non-builtin");
  switch (v.type.builtin.kind) {
    case type::kind_enum::i8: {
      auto nv = as_num2(v);
      int64_t s = std::holds_alternative<int64_t>(nv)
                      ? std::get<int64_t>(nv)
                      : static_cast<int64_t>(std::get<uint64_t>(nv));
      return llvm::ConstantInt::get(llvm::Type::getInt8Ty(ctx_), s, true);
    }
    case type::kind_enum::u8: {
      auto nv = as_num2(v);
      uint64_t u = std::holds_alternative<uint64_t>(nv)
                       ? std::get<uint64_t>(nv)
                       : static_cast<uint64_t>(std::get<int64_t>(nv));
    ...
    case type::kind_enum::string:
      ...
      auto* gv =
          new llvm::GlobalVariable(*mod_, arr->getType(),
                                   true,  // isConstant
      ...
      return llvm::ConstantExpr::getInBoundsGetElementPtr(arr->getType(), gv,
                                                          idxs);
  }
